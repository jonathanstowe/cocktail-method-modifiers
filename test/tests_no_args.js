QUnit.module("Modifier No Arguments");
test("before modifier", function() {
	var obj;
   ok(obj = new ModifierNoArgs(), "new object with before modifier");
   var ret;
   ok(ret = obj.before_meth(), "method with modifier");
...
test("after modifier", function() {
	var obj;
   ok(obj = new ModifierNoArgs(), "new object with after modifier");
   var ret;
   ok(ret = obj.after_meth(), "method with modifier");
   ok(typeof ret === 'object', "got an array ");
	ok(ret.length == 2,"got an array with two items");
...
test("around modifier", function() {
	var obj;
   ok(obj = new ModifierNoArgs(), "new object with around modifier");
   var ret;
   ok(ret = obj.around_meth(), "method with modifier");
   ok(typeof ret === 'object', "got an array ");
